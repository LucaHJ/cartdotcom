...
`POST /api/control/recover/{failed_run_id}` uses the existing dashboard authentication. It idempotently creates a NEW run linked to the failed source; the original record and archive remain untouched. Validated industry screens from a compatible failed run no older than 72 hours can be reused. Synthesis and challenge can also be reused when their complete dependency chain is reusable and every stage is at most 24 hours old. Reused evidence retains its original date and source usage (not double-counted in the new run). Portfolio context and final allocation are always refreshed; reused analysis is evidence, never authority to trade. No old decisions/orders are restored. The new explicitly requested run has its own three-hour budget. Manual recovery may research on weekends, but never changes market-hour execution restrictions or queue expiry.

Allocation recovery v4 explicitly represents cash only in `cash_target_pct`, not industry or position records. Errors report the offending industry's declared and calculated totals. Allocation permits two transient failures and two invalid drafts, with a maximum of four process launches inside the existing deadline; exhausting either failure budget stops the run. A rejected draft is supplied as a local read-only JSON file for correction. Full saved prior-stage results are also supplied in `prior-research.json`, avoiding both huge repeated prompts and false claims that excerpted evidence is unavailable. Holdings/target-action checks run within the correction loop as well as at the API boundary. Every SELL requires a structured review comparing retention, replacement and cash; funding sales must name actual final BUYs, while standalone exits must explain why their thesis survives rejected replacements. This is an auditable reasoning requirement, not a guarantee of investment quality or forced trading.
