...
export async function onRequestPost(context) {
    ...
    if (!token) return json({ error: "Reel Brain access is not configured." }, 503);
    const workerUrl = normalizeString(context.env.REEL_BRAIN_WORKER_URL, 500) || DEFAULT_WORKER_URL;
    const response = await fetch(new URL("/integration/reel-library/instagram-browser/complete", workerUrl), {
        method: "POST",
