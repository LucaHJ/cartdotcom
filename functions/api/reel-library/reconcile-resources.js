...
export async function onRequestPost(context) {
    ...
    const response = await fetch(`${workerUrl}/integration/reel-library/reconcile-resources`, {
        method: "POST",
        headers: { authorization: `Bearer ${token}`, "content-type": "application/json" },
        body: JSON.stringify({ after, limit: 24 })
    });
    const payload = await response.json().catch(() => ({}));
    if (!response.ok || payload.error) return json({ error: payload.error || "Resource pages could not be reconciled." }, response.status || 502);
