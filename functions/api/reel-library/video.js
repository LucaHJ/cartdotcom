...
export async function onRequestGet(context) {
    ...
    if (!token) return json({ error: "Archived-video access is not configured." }, 503);
    const workerUrl = normalizeString(context.env.REEL_BRAIN_WORKER_URL, 500) || DEFAULT_WORKER_URL;
    const response = await fetch(`${workerUrl}/integration/reel-library/jobs/${encodeURIComponent(jobId)}/video`, {
        method: "POST",
        headers: { authorization: `Bearer ${token}` }
    });
    const payload = await response.json().catch(() => ({}));
    if (!response.ok || payload.error) return json({ error: payload.error || "Archived video could not be opened." }, response.status || 502);
