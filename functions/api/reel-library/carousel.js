...
export async function onRequestGet(context) {
    ...
    if (!/^[0-9a-f-]{36}$/i.test(jobId)) return json({ error: "A valid carousel job id is required." }, 400);
    const token = normalizeString(context.env.REEL_LIBRARY_SHARED_TOKEN, 4000);
    if (!token) return json({ error: "Archived-carousel access is not configured." }, 503);
    const workerUrl = normalizeString(context.env.REEL_BRAIN_WORKER_URL, 500) || DEFAULT_WORKER_URL;
    const response = await fetch(`${workerUrl}/integration/reel-library/jobs/${encodeURIComponent(jobId)}/carousel`, {
        method: "POST",
        headers: { authorization: `Bearer ${token}` }
    });
    const payload = await response.json().catch(() => ({}));
    if (!response.ok || payload.error) return json({ error: payload.error || "Archived carousel could not be opened." }, response.status || 502);
